...
//                if ($user['user_money'] < $withdrawals['money']) {
                if ($user['y_money'] < $withdrawals['money']) {
                    $this->error("用户余额不足{$withdrawals['money']}，不够提现");
                    exit;
                }
                accountLog($withdrawals['user_id'], ($withdrawals['money'] * -1), 0, "佣金提现", $withdrawals['money']);
                $remittance = array(
            ...
            } elseif ($model->status == 2) { //拒绝提现
                // 回滚金额
                $sql = "UPDATE __PREFIX__users SET out_money = out_money + {$user['y_money']},y_money = y_money - {$user['y_money']} WHERE user_id = {$withdrawals[user_id]} ";
                M()->execute($sql);
                /* 插入帐户变动记录 */
                $account_log = array(
                    'user_id' => $withdrawals['user_id'],
                    'user_money' => $withdrawals['money'],
                    'pay_points' => 0,
                    'change_time' => time(),
                    'desc' => '拒绝提现',
                );
        ...
        if ($user['nick_name'])
            $withdrawals['user_name'] = $user['nick_name'];
        elseif ($user['email'])
