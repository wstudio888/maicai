...
class StoreController extends BaseController {
    ...
    public function getStores(){
    	$store_class = M('store')->where(array('store_state'=>1))->select();
    ...
    public function searchStoreGoodsClass(){
        ...
        $list = M("goods")->where("store_id = $store_id")->field("goods_remark,goods_content" , true)->limit(0 , 10)->select();// ->query("select *  from __PREFIX__goods $where $limit ");
    ...
    public function storeGoods()
    {
        ...
        //获取店铺信息
        $store = M('store')->where(array("store_id"=>$store_id,'store_state'=>1))->field("store_id,store_name,store_address,store_logo")->find();
        ...
        if($count > 0)
        {
        	$goods_list = M('goods')->field('goods_id,cat_id2,goods_sn,goods_name,shop_price,comment_count,original_img,sales_sum')->where("store_id = $store_id and is_on_sale = 1")->order("$sort $sort_asc")->limit($page->firstRow.','.$page->listRows)->select();
