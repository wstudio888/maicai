...
var oldNick = $("#nick_nameInput").val();
(function () {
    var revisenick_name = $('.reg-nick_name-revise'),
        regnick_name = $('#changenick_name');
    var usernamePrompt = {
    ...
    regnick_name.click(function () {
        var self = $(this);
        $("#username_error").empty();
        self.parent().hide();
        revisenick_name.show().focus();
        return false;
    });
    //focus
    revisenick_name.find('.text').focus(function () {
        $(this).addClass('hover');
    ...
    revisenick_name.find('.text').blur(function () {
        $(this).removeClass('hover');
        var uError = $('#username_error');
        uError.html('');
    });
    //nick_name save
    revisenick_name.find('.j_save').click(function () {
        nick_nameParentNode = regnick_name.parent();
        var nick_name = revisenick_name.find('.text').val();
        var username = $.trim(nick_name);
        if (username == oldNick) {
            $("#orgNick").html(username);
            nick_nameParentNode.show();
            revisenick_name.hide();
            oldNick = username;
            return;
        }
        ...
        $.getJSON("../validate/newnick_name?nick_name=" + escape(username) + "&k=" + getKey() + "&r=" + Math.random(), function (date) {
            if (date.success == 1) {
                $("#orgNick").html(username);
        ...
        $.ajax({
            ...
            success: function (result) {
                if (result) {
                    ...
                    if (obj.rs == -7) {
                        mobileCodeError("������֤���ֻ����뵽<a href='http://safe.jd.com/user/paymentpassword/safetyCenter.action' class='emreg-nick_name'>�˻���ȫ</a>��鿴��");
                    }
                    if (obj.info) {
    ...
    $('#toValidate').click(function () {
        ...
        $.getJSON("../reg/validateMobile?" + data, function (result) {
                ...
                if (result.success == -5) {
                    clientError("������֤���ֻ����뵽<a href='http://safe.jd.com/user/paymentpassword/safetyCenter.action' class='emreg-nick_name'>�˻���ȫ</a>��鿴��");
                    return;
                }
                if (result.success == -7) {
                    window.location.href = "http://reg.jd.com/reg/expire";
