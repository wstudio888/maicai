...
class OrderController extends BaseController
{
    ...
    */
    ...
    public function deliveryHandle(){
        ...
        $data['shop_no'] = '11047059'; //门店ID
        
        // 查询订单信息
        $order_info           = M('order')->field('consignee,order_amount,mobile')->where(['order_id' => $data['order_id'], 'store_id' => $store_id])->find();
        $data['consignee']    = $order_info['consignee'];
        $data['order_amount'] = $order_info['order_amount'];
        $data['mobile']       = $order_info['mobile'];
        
        //查询达达订单有没有
        $dada_status = M('dada_order')->where(array('order_id' => $data['order_id']))->count();
        
        $dadaLogic = new DadaLogic();
        $res       = $dadaLogic->add_order($data, $dada_status);
        
        if (!$res) $this->error('操作发货失败');
