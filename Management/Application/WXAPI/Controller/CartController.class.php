...
*/
    ...
    public function cart2(){
        ...
        // 获取商家的快递
//      SELECT DISTINCT shipping_code,store_id FROM `ty_shipping_area` where  is_close = 1 and store_id in (1,2) ;
        $shipping_arr   = [];
        $store_shipping = M('shipping_area')->where(array('is_close' => 1, 'store_id' => array('in', $store_id_arr)))->field(array('DISTINCT shipping_code', 'store_id'))->select();
        foreach ($store_shipping as $k => $v) {
            $shipping_arr[$v['store_id']][$k]['code'] = $v['shipping_code'];
            foreach ($shippingList as $kk => $vv) {
                if ($vv['code'] == $v['shipping_code']) {
                    $shipping_arr[$v['store_id']][$k]['name'] = $vv['name'];
                    $shipping_arr[$v['store_id']][$k]['code'] = $vv['code'];
                }
            }
        }
        ...
        where c2.uid = {$this->user_id}  and c1.use_start_time < " . time() . " AND " . time() . " < c1.use_end_time and c1.condition <= {$cart_result['total_price']['total_fee']} and c2.store_id in (" . implode(',', $store_id_arr) . ")";
        $couponList = $Model->query($sql);
        array_push($couponList, array('name' => '不使用优惠卷', 'money' => 0));
        // 收货地址
    ...
    public function cart3(){
        //$unique_id = I("unique_id"); // 唯一id  类似于 pc 端的session id
        $this->user_id = I("get.user_id"); // 用户id
        $usersInfo     = get_user_info($this->user_id);  // 用户
        $address_id    = I("get.address_id"); //  收货地址id
//        $shipping_code = array('1' => 'shentong'); //  物流编号
//      $shipping_code    = I("shipping_code"); //  物流编号 //  物流编号
        
        $invoice_title    = I('get.invoice_title', "无"); // 发票
        $couponTypeSelect = array(1); //  优惠券类型  1 下拉框选择优惠券 2 输入框输入优惠券代码
        $coupon_id        = I("get.coupon_id", 0); //  优惠券id
        $couponCode       = array(); //  优惠券代码
        $pay_points       = I("get.pay_points", 0); //  使用积分
        $user_money       = I("get.user_money", 0); //  使用余额
        $user_money       = $user_money ? $user_money : 0;
        $user_note        = array(); // $user_note = I('user_note'); // 给卖家留言      数组形式
        $object           = file_get_contents('php://input');
        $_POST            = (json_decode($object, true));
        $shipping         = $_POST['shipping'];
        $shipping_code    = array_filter($shipping);
        ...
        // 获取所有商铺ID
        $store_id_arr = M('cart')->where("user_id = {$this->user_id} and selected = 1")->group("store_id")->getfield("store_id", true);
        // 查询商铺是否开启快递
        $store_shipping = M('shipping_area')->where(array('is_close' => 1, 'store_id' => array('in', $store_id_arr)))->field(array('DISTINCT shipping_code', 'store_id'))->count();
        if ($store_shipping) {
            if (!count($shipping_code)) exit(json_encode(array('status' => -1, 'msg' => '请选择物流信息', 'result' => null))); // 返回结果状态
        } else {
            $shipping_code = null;
        }
    ...
    var $config = array(
        'appid' => "wxf3a9ae58645327d5",    /*微信开放平台上的应用id*/
        'mch_id' => "1486945902",   /*微信申请成功之后邮件中的商户id*/
        'api_key' => "gdwskjyxgs1382773589313827735893",    /*在微信商户平台上自己设定的api密钥 32位*/
        'notify_url' => SITE_URL . '/index.php/WXAPI/Pay/wxCallback.html' /*自定义的回调程序地址id*/
    );
