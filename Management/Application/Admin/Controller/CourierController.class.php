...
class CourierController extends BaseController
{
    ...
    public function addcourier(){
        ...
        if($data){
            $data['courier_pwd'] = md5($data['courier_pwd']);
            $data['courier_idle'] = 1;
            $r = D('courier')->add($data);
            if($r){
                $this->success("操作成功",U('Admin/courier/courier'));
            }else{
    ...
    public function apply_del(){
        ...
        M('courier')->where(array('courier_id' => $id))->delete();
        $this->success('操作成功', U('Admin/courier/courier'));
    }

    //修改快递员状态
    public function change(){
        D('courier')->where(array("courier_id" => $_GET['courier_id']))->save($_GET);
    }
}