...
//        $user_id = I('user_id',0);
    ...
    public function getGoodsCollect(){
        ...
        // 店铺列表
          $store= M("StoreCollect")
                ->alias('a')
                ->field('a.store_id,a.user_id,a.add_time,b.store_logo,b.store_name,b.store_address')
                ->join('left join ty_store AS b ON a.store_id = b.store_id')
                ->where("a.user_id in($this->user_id)")
                ->cache(false,TPSHOP_CACHE_TIME)
                ->select();
            foreach($store as $k => $v){
                $store[$k]['store_logo'] = SITE_URL.$v['store_logo'];
            }
            $data['store'] = $store;
        unset($data['show']);
        exit(json_encode($data));
    ...
    public function getOrderFind(){
        ...
        if (!$this->user_id) exit(json_encode(array('status' => -1, 'msg' => '缺少参数', 'result' => '')));
        $map        = array('user_id' => $this->user_id, 'order_id' => $orderId, 'order_status' => '2', 'is_comment' => 0);
        $order_list = M('order')->where($map)->find();
        $data       = $this->userLogic->get_order_goods($order_list['order_id']);
    ...
    public function getOrderDetail(){
        ...
        empty($order_info['shipping_price']) && $order_info['shipping_price'] = 0;
    ...
    public function register(){
        ...
        $user               = M('users');
        $distributor_status = M('config')->where(array('name' => 'distributor'))->getfield('value');
        if ($distributor_status) {
            //查询分销ID的用户是否存在
            $fxUser               = $user->where(['user_id' => $data['fid']])->field(['user_id', 'first_leader', 'second_leader', 'third_leader'])->find();
            $data['is_distribut'] = 1;
            if ($fxUser) {
                $data['first_leader']  = $fxUser['user_id'];        //一级
                $data['second_leader'] = $fxUser['first_leader'];   //二级
                $data['third_leader']  = $fxUser['second_leader'];  //三级
            }
        }
        $id  = $user->add($data);
        $res = $user->where(array("user_id" => $id))->find();
    ...
    public function getOpenid(){
        $result = curl_get($_GET['url']);
        echo $result;
    }
    
    public function checkrwm(){
        $user_id = I('get.user_id', 0, 'intval');
        if (!$user_id) exit(json_encode(array('status' => -1, 'msg' => '缺少参数', 'result' => '')));
        $wx_rwm = S('wx_rwm'. $user_id );
        if ($wx_rwm) {
            echo json_encode(array("code" => '200', 'msg' => '获取二维码成功', 'data' => SITE_URL . $wx_rwm));
            die;
        }
        $wx_rwm = M('users')->where(['user_id' => $user_id])->getfield('wx_rwm');
        S('wx_rwm' . $user_id, $wx_rwm);
        if (!$wx_rwm) {
            $access_token = $this->getAccess_Toke();
            $imgPath      = $this->goRwm($user_id, $access_token); //获取二维码地址
            if ($imgPath) {
                if (M('users')->where(['user_id' => $user_id])->save(['wx_rwm' => $imgPath]) !== false) {
                    echo json_encode(array("code" => '200', 'msg' => '数据库获取二维码成功', 'data' => SITE_URL . $imgPath));
                    die;
                }
            } else {
                echo json_encode(array("code" => '400', 'msg' => '获取二维码图片失败'));
                die;
            }
        }
        echo json_encode(array("code" => '200', 'msg' => '缓存获取二维码成功', 'data' => SITE_URL . $wx_rwm));
    }
    
    private function getAccess_Toke(){
        $access_token = S('access_token');
        if ($access_token) {
            return $access_token;
        }
        //微信获取
        $url    = "https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid=wxf3a9ae58645327d5&secret=c90e9c7fa14d4df6fbde0970cc2f0fb0";
        $result = curl_get($url);
        S('access_token', json_decode($result, true), 7200);
        return json_decode($result, true);
    }
    
    private function goRwm($user_id, $access_token){
        ...
        $imgPath = './Public/upload/wx/' . $user_id . '.jpg';
        wx_curl_post($url, ['path' => $path, 'width' => 300], $imgPath);
        Vendor('Img.Imcompress');
        (new \Imcompress($imgPath, 1))->compressImg($imgPath);
        return substr($imgPath, 1, strlen($imgPath));
    }
    
    public function getUserFxInfo(){
        $user_id = I('get.user_id', 0, 'intval');
        empty($user_id) && $this->error('参数错误!');
        
        $user     = M('users');
        $userInfo = $user->where(array('user_id' => $user_id))->find();
        empty($userInfo) && $this->error('参数错误!');
        
        $data = array();
        //推荐人
        $data['first_leader'] = $userInfo['first_leader'];
        //成功提现金额
        $data['ok_money'] = $userInfo['ok_money'];
        //可提现金额
        $data['out_money'] = $userInfo['out_money'];
        //累计分销佣金
        $data['distribut_money'] = $userInfo['distribut_money'];
        //分销订单
        $data['rebate_count'] = M("rebate_log")->where(array('user_id' => $user_id))->count();
        //提现明细
        $data['detailed'] = M('detailed')->where(array('user_id' => $user_id))->count();
        //我的下线
        $where           = " first_leader = $user_id or second_leader = $user_id or third_leader = $user_id";
        $data['my_user'] = $user->where($where)->count();
        
        if ($userInfo['first_leader']) {
            $data['first_leader'] = $user->where(array('user_id' => $userInfo['first_leader']))->getfield('nick_name');
        } else {
            $data['first_leader'] = '无';
        }
    ...
    public function textok(){
        //echo $address_id;
        $object        = file_get_contents('php://input');
        $_POST         = (json_decode($object, true));
        $this->user_id = I('get.user_id', 0);
        $rec_id        = I('get.rec_id');
        $textcontent   = $_POST['textcontent'];
        
        if (!$this->user_id || !$rec_id) exit(json_encode(array('status' => -1, 'msg' => '缺少参数', 'result' => '')));
        
        if ($textcontent && mb_strlen($textcontent, 'utf8') < 500) {
            $order = M('order_goods')->alias('g')->where(array('user_id' => $this->user_id, 'g.rec_id' => $rec_id, 'g.is_comment' => 0))
                                     ->join("INNER JOIN  " . C('DB_PREFIX') . "order as o on g.order_id = o.order_id ")->find();
            if (!$order) {
                exit(json_encode(array('status' => -1, 'msg' => '商品已评论', 'result' => '')));
            }
            $data               = array();
            ...
            if (M('comment')->add($data) !== false) {
                M('order_goods')->where(array('rec_id' => $rec_id))->save(array('is_comment' => 1));
                //检查下面是否还有没评价的
                $checocomment = M('order_goods')->where(array('order_id' => $order['order_id'], 'is_comment' => 0))->count();
                if (!$checocomment) {
                    M('order')->where(array('user_id' => $this->user_id, 'order_id' => $order['order_id'], 'is_comment' => 0))->save(array('is_comment' => 1));
                }
                exit(json_encode(array('status' => 200, 'msg' => 'ok', 'result' => '')));
            }
        } else {
            exit(json_encode(array('status' => -1, 'msg' => '内容为空或超过字数限制', 'result' => '')));
        }
    }
    ...
    public function customer(){
        ...
        //查询订单确认收货
        M('order')->where(['order_id' => $id, 'order_status' => 2, ''])->select();
    ...
    public function offline(){
        ...
        $Page->firstRow = $Page->listRows * $page;
        $select         = "(select count(*) from " . C('DB_PREFIX') . "order as o where o.user_id = u.user_id) as ordercount ";
        $field          = array('u.nick_name', 'u.user_id', 'head_pic', 'reg_time', $select);
        
        $order_list  = M('users')->alias('u')->where($map)->field($field)->order($order_str)->limit($Page->firstRow . ',' . $Page->listRows)->select();
        $user_id_arr = get_arr_column($order_list, 'user_id');
        ...
        foreach ($order_list as $key => $userlist) {
            $order_list[$key]['usersum']  = $first_leader[$userlist['user_id']]['count'] + $second_leader[$userlist['user_id']]['count'] + $third_leader[$userlist['user_id']]['count'];
            $order_list[$key]['reg_time'] = date("Y-m-d H:i:s", $userlist['reg_time']);
            $order_list[$key]['head_pic'] = DSHOPS_URL . '/' . $userlist['head_pic'];
        }
    ...
    public function detailed(){
        ...
        if (I('type'))
            $count = M('withdrawals')->where($map)->count();
        $Page = new \Think\Page($count, 10);
        
        $show           = $Page->show();
        $order_str      = "create_time DESC";
        $Page->firstRow = $Page->listRows * $page;
        $field          = array('create_time', 'money', 'account_bank', 'account_name', 'bank_name', 'status', 'remark');
        
        $statusString = C('WITH_DRAWALS');
        $order_list   = M('withdrawals')->where($map)->field($field)->order($order_str)->limit($Page->firstRow . ',' . $Page->listRows)->select();
        foreach ($order_list as $key => $userlist) {
            $order_list[$key]['create_time']  = date("Y-m-d H:i", $userlist['create_time']);
            $order_list[$key]['statusString'] = $statusString[$userlist['status']];
        }
    ...
    public function fxOrderList(){
        ...
        //找到下级分销所有人员
        $userlisd = M('users')->where("first_leader = $this->user_id or second_leader = $this->user_id or third_leader = $this->user_id")->getfield('user_id', true);
        
        if (!$userlisd) {
            exit(json_encode(array('status' => 1, 'msg' => '获取成功', 'result' => null)));
        }
        $userlisd = implode(",", $userlisd);
        
        $map = '';
        $map .= " user_id in ($userlisd) ";
        if ($type == 1) {
            $map .= " AND pay_status = 0 AND order_status = 0 AND pay_code !='cod' ";
        } elseif ($type == 2) {
            $map .= ' AND (pay_status = 1 OR pay_code="weixin")  AND order_status = 1 ';
        } elseif ($type == 3) {
            $map .= 'AND ( order_status = 2 or order_status = 4 )';
        } else {
            $map .= ' AND pay_status = 0 AND order_status = 0 AND pay_code !="cod" ';
        }
        //echo 1;
    ...
    public function getUserInfoMoney(){
        ...
        $userinfo = M('users')->field('ok_money,out_money,distribut_money,d_money,p_money,w_money,y_money')->where(array('user_id' => $this->user_id))->find();
        if ($userinfo) {
            // 未结算佣金
            exit(json_encode(array('status' => 1, 'msg' => '获取成功', 'result' => $userinfo)));
        } else {
            exit(json_encode(array('status' => 1, 'msg' => '获取成功', 'result' => null)));
        }
    }
    ...
    public function getcoupon(){
        ...
        if (!$this->user_id || !$couponid) exit(json_encode(array('status' => -1, 'msg' => '缺少参数', 'result' => '')));
        $coupon = M('coupon');
        // 查询优惠卷是否可以领取
        $info = $coupon->where(['type' => 1, 'send_start_time' => array('LT', $time), 'send_end_time' => array('GT', $time)])->find();
        if (empty($info) || $info['total'] <= $info['giveout']) { //代金券不存在或者已兑换完
            exit(json_encode(array('status' => -2, 'msg' => '代金券已兑换完', 'result' => null)));
        }
        // 买家最多只能拥有同一个店铺尚未消费抵用的店铺代金券最大数量的验证
        $ucount = M('coupon_list')->where(['cid' => $couponid, 'uid' => $this->user_id])->count();
        if ($ucount >= $info['eachlimit']) {
            $msg = sprintf('该代金券您已兑换%s次,不可再兑换了', $info['eachlimit']);
            exit(json_encode(array('status' => -2, 'msg' => $msg, 'result' => null)));
        }
        // 添加代金劵信息
        $data              = array();
        $data['cid']       = $couponid;
        ...
        if (M('coupon_list')->add($data) !== false) {
            exit(json_encode(array('status' => 1, 'msg' => '获取代金劵成功',)));
        }
    }
