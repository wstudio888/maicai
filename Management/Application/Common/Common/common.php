...
//           " pay_points = pay_points + $pay_points, distribut_money = distribut_money + $distribut_money WHERE user_id = $user_id";
      $sql = "UPDATE __PREFIX__users SET y_money = y_money + $user_money,ok_money = ok_money + $distribut_money," .
             " pay_points = pay_points + $pay_points, distribut_money = distribut_money + $distribut_money WHERE user_id = $user_id";
...
function confirm_order($id, $user_id = 0){
    ...
    $rebate = M('rebate_log')->where("order_id = $id and status = 2")->select();
    $user   = M('users');
    // 累计未结算金额
    foreach ($rebate as $key => $value) {
        $user->where(['user_id' => $rebate['user_id']])->setInc('d_moeny', $rebate['money']);
    }
