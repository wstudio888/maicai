...
class CommentController extends BaseController
{
    ...
    public function ajaxindex(){
        ...
        $comment_list = $model->field('c.*,u.nick_name as nick_name')->table(C('DB_PREFIX') . 'comment c')->join('LEFT JOIN __USERS__ u ON u.user_id = c.user_id')->where($where)->order('add_time DESC')->limit($Page->firstRow . ',' . $Page->listRows)->select();
    ...
    public function subject_info(){
        if (IS_POST) {
            ...
            if (M('complain_subject')->add($data)) {
                $this->success('添加成功', U('Comment/subject_list'));
                exit;
            } else {
